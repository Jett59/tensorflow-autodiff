...
class Model:
    ...
    def combined_loss(self, input_values, expected_values):
        return tensorflow.reduce_mean(
            tensorflow.vectorized_map(
                lambda x: self.loss(x[0], x[1]), (input_values, expected_values)
            )
        )
    ...
    def train(self, inputs, expected_outputs, optimizer, batch_size=32, epochs=10):
        ...
        batched_inputs = tensorflow.reshape(
            inputs[: evenly_divisible_batch_count * batch_size],
            (evenly_divisible_batch_count, batch_size, inputs.shape[1]),
        )
        batched_expected_outputs = tensorflow.reshape(
            expected_outputs[: evenly_divisible_batch_count * batch_size],
            (evenly_divisible_batch_count, batch_size, expected_outputs.shape[1]),
        )
        ...
        for i in range(epochs):
            ...
            loss = self.combined_loss(batched_inputs[0], batched_expected_outputs[0])
            for batch_index, (batched_input, batched_expected_output) in enumerate(
                zip(
                    tensorflow.unstack(batched_inputs),
                    tensorflow.unstack(batched_expected_outputs),
                )
            ):
                loss = self.training_step(
                    variables, batched_input, batched_expected_output, optimizer
                )
            end_time = time.time()
            print("Loss: %s (in %s seconds)" % (loss.numpy(), end_time - start_time))
            if hasattr(optimizer, "on_epoch_complete"):
                optimizer.on_epoch_complete()
